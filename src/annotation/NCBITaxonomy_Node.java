...
public class NCBITaxonomy_Node {
	...
	public static HashSet<String> getAllLeafSpeciesForNode(NCBITaxonomy_Node node){
		...
		if(node.hasChildren()){
			...
			for(NCBITaxonomy_Node thisNode: node.getChildren())
				result.addAll(getAllLeafSpeciesForNode(thisNode));
		}else{
	...
	public void printSummary(String indents, int minReads){
		...
		if(_inheritedReadCount >= minReads  ||  _directReadCount >= minReads){
			//System.out.println(indents+"\t"+(indents.length()-1)+"\t"+_nodeLevel+"\t"+this.getNodeIndex()+"\t"+this.getParent().getNodeIndex()+"\t"+_taxonomyName+"\t"+_directReadCount+"\t"+_inheritedReadCount);
			int parentIndex = -1;
			if(this.hasParent())
				parentIndex = this.getParent().getNodeIndex();
			System.out.println((indents.length()-1)+"\t"+_nodeLevel+"\t"+this.getNodeIndex()+"\t"+parentIndex+"\t"+_taxonomyName+"\t"+_directReadCount+"\t"+_inheritedReadCount);
